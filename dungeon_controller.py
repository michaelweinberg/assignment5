...
class DungeonController:
    ...
    def draw_hero(self):
        hero_move = self.__dungeon.__movement_list[-1]
        pre_move = self.__dungeon.__movement_list[-2]
        self.__dungeon.__hero.hero_move(self.__dungeon.__movement_list)
        print(self.__dungeon.__hero.get_health())
        self.draw_cell(hero_move.get_y(), hero_move.get_x(), "#232323")
        if pre_move == self.__dungeon.__map.start_point:
            self.draw_cell(pre_move.get_y(), pre_move.get_x(), "#eee83f")
        elif pre_move == self.__dungeon.__map.destination:
            self.draw_cell(pre_move.get_y(), pre_move.get_x(), "#cf52eb")
        elif pre_move != self.__dungeon.__map.start_point and pre_move != self.__dungeon.__map.destination:
    ...
    def check_reach(self):
        if self.__dungeon.__movement_list[-1] == self.__dungeon.__map.destination:
            if self.__dungeon.__hero.get_pillars():
    ...
    def move_west(self):
        x = self.__dungeon.__hero.get_x()
        y = self.__dungeon.__hero.get_y()
        print("hero(y,x)", y, x)
        room = self.__dungeon.__map.get_room(y, x - 1)
        if self.__dungeon.__map.is_movable(room):
            self.__dungeon.__hero.set_x(x - 1)
            self.__dungeon.__movement_list.append(room)
            self.draw_hero()
    ...
    def move_south(self):
        ...
        y = self.__dungeon.__hero.get_y()
        print("hero(y,x)", y, x)
        room = self.__dungeon.__map.get_room(y + 1, x)
        if self.__dungeon.__map.is_movable(room):
            self.__dungeon.__hero.set_y(y + 1)
            self.__dungeon.__movement_list.append(room)
            self.draw_hero()
